...
namespace triton::arch::x86 
{
  ...
  void x8664Cpu::disassembly(triton::arch::Instruction& inst) 
  {
    ...
    /* Clear instruction's operands if already defined */
    inst.operands.clear();
    ...
    /* Let's disasm and build our operands */
    count = triton::extlibs::capstone::cs_disasm(
      this->handle, 
      inst.getOpcode(), 
      inst.getSize(), 
      inst.getAddress(),
  ...
  triton::uint8 x8664Cpu::getConcreteMemoryValue(triton::uint64 addr, bool execCallbacks) const 
  {
    ...
    return (it == this->memory.end()) ? 0 : it->second;
  }
