...
:: Forked from: https://github.com/0xeb/useful-scripts

:: Check Triton environment file and pick first the one from the current directory then from the script directory
...
if not exist "%TRITON_ENV_FILE%" (
  echo Triton environment file not found. Make a copy of 'triton.template.env' and customize it.
  goto :eof
)

:: Apply environment variables from the Triton environment file
